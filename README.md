...
- **`associate_degree`** — what a transfer associate degree waives. The catalog says a qualifying
  associate degree meets lower-division general education *"except those specified as major or
  college requirements and requirements for completion of the undergraduate writing program"*, so
  the option waives 11 categories as a single record and leaves Written Communication and the
  upper-division block alone.
